# Val Ark on UT2 (Rockchip RK3588 NAS appliance)

Setup notes, gotchas, and fixes from bringing Val Ark up on a **UT2** unit — a
Rockchip **RK3588** (aarch64, Cortex-A55 ×8) NAS appliance running **ZOS** (a
Debian 11 based vendor OS) with an on-board **NPU** and a RealSense camera.

This is the reference for "what goes wrong on a UT2, and how it's handled."
Capacities and host-specific paths vary per unit — set yours in `.env`.

## TL;DR platform

| Part | Detail |
|------|--------|
| SoC | Rockchip RK3588, 8× Cortex-A55, aarch64 |
| NPU | RKNPU driver v0.9.x; `rknn_toolkit_lite2` in system Python |
| Camera | RealSense (vendor `nas-dashboard.service` runs an NPU webapp) |
| OS | ZOS (`ID=debian`, bullseye); NAS stack: openresty, zfsv3, storage-pool, samba |
| Boot disk | small eMMC, ext4 — do NOT fill |
| Bulk storage | two large NVMe btrfs volumes (`/data_n001`, `/data_n002`) |
| md-RAID1 | Only swap and a small system partition are mirrored |

## Storage reality (read before touching disks)

The large NVMe volumes are **NOT block-level RAID1**. They are two independent
single-device btrfs filesystems. The appliance pools them into one namespace with
a FUSE union called **`zfsv3`** (mounted at `/tmp/zfsv3`) whose branches are the
`data/udata/real/` dirs on each drive. That union also holds the NAS **user
shares**, the **app store**, **InfluxDB**, and the **RealSense/NPU dashboard**.
...
*beside* the appliance data (outside the `zfsv3` union), and can spread across
both drives for maximum capacity:

| Val Ark data | Drive | Why |
|--------------|-------|-----|
| Tree (`tools/content/sources/...`) | 1 (`/data_n001/val-ark`) | native btrfs: atomic rename, symlinks, NFS export |
| Models | 1 (`/data_n001/models`) | sibling of the tree, reuses the `~/models` convention |
| **SeaweedFS blobs** | 2 (`/data_n002/seaweedfs`) | second NVMe → both drives used, spreads I/O |
...
- `VALARK_MAX_GB=3000` — total Val Ark data (tools + models + content) tops out at
  ~3 TB. The librarian computes its fill budget as `min(disk headroom, cap − usage)`
  and stops there; `librarian status` shows the cap, current usage, and budget left.
- `VALARK_MODEL_MAX_GB=10` — the planner skips any single model larger than this, so
  the fill stays on **apps + small models** and never pulls a huge flagship model.

Both are enforced in `scripts/librarian.sh` (verified: a plan tops out at ~3 TB and
the model filter drops oversized candidates).
...
2. **Data-root autodetect misses the NVMe.** `valark-env.sh` autodetect scans
   `/mnt/*`, `/data`, `/srv/val-ark`, `/var/lib/val-ark` — none match this box's
   `/data_n001` / `/data_n002`, so it would fall back to the **boot eMMC** and the
   Librarian would try to fill the system disk. **Fix:** set `VAL_ARK_DATA` in
   `.env` (done). *Recommendation:* teach autodetect to also consider the largest
   writable non-system mount so other appliances aren't silently wrong.
...
4. **Node.js not installed** — the zero-dep web server (`scripts/server.js`) needs
   ...
   `~/.local/node` (where `start.sh` already looks). Arch-aware (`linux-arm64` here);
   override the version with `VALARK_NODE_VERSION`.
...
6. **SeaweedFS default volume port 8080 collides with the appliance.** The ZOS
   `openresty` NAS web stack already listens on `127.0.0.1:8080`, so
...
9. **Headless browser screenshots can't run here.** The vendor `chromium` has no
   working headless/ozone backend (`Invalid ozone platform: headless`), there's no
   `xvfb`, and no Playwright browsers are installed — so the Playwright screenshot
   and UI-test suite (`tests/screenshots/`) can't run *on the UT2*. The **bash
...
This box is genuinely capable of on-NPU vision inference, and Val Ark already
leans toward it:

- **Hardware/runtime present:** RKNPU driver + `rknn_toolkit_lite2` (Python
  `rknnlite` imports cleanly). The vendor ships a working YOLO-on-NPU demo
  (`bus.jpg`, `coco_80_labels_list.txt`) plus a RealSense dashboard.
...
#   VALARK_SEAWEED_VOLUME_PORT=8085            (avoid openresty on 8080)
