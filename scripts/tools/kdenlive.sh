...
download_kdenlive() {
    ...
    download_file "$url" "${dest}/Kdenlive.AppImage" "kdenlive linux-x86_64"
...
"
    ...
    download_file "$url" "${dest}/Kdenlive.dmg" "kdenlive macos-arm64"

    # windows-x64: installer
    dest="${TOOLS_DIR}/windows-x64/kdenlive"
    ensure_dir "$dest"
    url="https://download.kde.org/stable/kdenlive/${major_minor}/windows/kdenlive-${ver}.exe"
    download_file "$url" "${dest}/Kdenlive-Setup.exe" "kdenlive windows-x64"
